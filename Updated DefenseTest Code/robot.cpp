...
void Robot::sensors()
{
    ...
    // -------------------------------------------------------
    // PERSON B: Signal 3 — Temporal consistency S_temp
    // Measures unexpected jump in estimated position vs odometry
    // -------------------------------------------------------
    {
        Vector2f cur_est = senses.position.mu;
        Vector2f expected_move = senses.velocity * sim->sp.dt;
        s_temp = (cur_est - prev_est_pos - expected_move).norm();
        prev_est_pos = cur_est;
    }

    // -------------------------------------------------------
    // PERSON B: Ensemble score S_inst
    // Sigmoid combination of all three signals
    // Weights: w1=1.0 (lambda), w2=0.05 (dkl), w3=5.0 (temp)
    // -------------------------------------------------------
    {
        const float w1 = 1.0f, w2 = 0.05f, w3 = 5.0f;
        const float threshold = 0.75f;
        float raw = w1 * s_lambda + w2 * s_dkl + w3 * s_temp;
        s_inst = 1.0f / (1.0f + expf(-raw + 5.0f)); // sigmoid centred at 5
        detector_flagged = (s_inst > threshold);
        if (s_inst > max_s_inst) max_s_inst = s_inst;

        // Log every 10 timesteps to keep file size manageable
        if (sim->sp.export_csv && ((int)(sim->total_time / sim->sp.dt) % 10 == 0))
        {
            detector_log.push_back({
                sim->total_time, s_lambda, s_dkl, s_temp, s_inst, detector_flagged
            });
        }

        // Reset per-timestep max signals (they accumulate in handle_blackboard)
        s_lambda = 0.0f;
        s_dkl    = 0.0f;
    }
    // -------------------------------------------------------
...
void Robot::render(DebugDraw *dd, Vector2f bias)
{
    ...
    {   
        ...
        // -------------------------------------------------------
        // PERSON B: Detector visual overlay
        // Red pulsing ring = flagged (s_inst > 0.75)
        // Orange ring      = elevated (s_inst > 0.4)
        // Ring thickness scales with s_inst
        // -------------------------------------------------------
        if (sim->sp.gui.detector)
        {
            if (detector_flagged)
            {
                // Bright red outer ring — confirmed detection
                dd->DrawCircle(rgt, radius * 1.8f,
                    b2Color(1.0f, 0.1f, 0.1f, 0.9f));
                dd->DrawCircle(rgt, radius * 2.1f,
                    b2Color(1.0f, 0.1f, 0.1f, 0.5f));
            }
            else if (s_inst > 0.4f)
            {
                // Orange ring — elevated suspicion
                dd->DrawCircle(rgt, radius * 1.6f,
                    b2Color(1.0f, 0.6f, 0.0f,
                        0.4f + 0.5f * (s_inst - 0.4f) / 0.35f));
            }
        }
        // -------------------------------------------------------
