...
public:
    ...
    // -------------------------------------------------------
    // PHASE 1: Attack parameters (Person A)
    // -------------------------------------------------------
    int         attack_type         = 0;
    Vector2f    attack_delta        = Vector2f(1.5f, 0.0f);
    float       attack_alpha        = 500.0f;
    int         attack_k_trigger    = 3;
    bool        attack_active       = false;

    // -------------------------------------------------------
    // PERSON B: Detector signals
    // -------------------------------------------------------
    float       s_lambda            = 0.0f;   // Signal 1: precision anomaly
    float       s_dkl               = 0.0f;   // Signal 2: KL divergence
    float       s_temp              = 0.0f;   // Signal 3: temporal consistency
    float       s_inst              = 0.0f;   // Ensemble score
    bool        detector_flagged    = false;  // True when s_inst > threshold
    Vector2f    prev_est_pos        = Vector2f::Zero();
    float       max_s_inst          = 0.0f;
    ...
    std::vector<DetectorRecord> detector_log;
    // -------------------------------------------------------
